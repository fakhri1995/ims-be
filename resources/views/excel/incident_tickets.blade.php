<table>
    ...
    <tr>
        ...
        @if($core_attributes[0])
            ...
            <th>Nama Engineer / Group</th>
            <th>Status Ticket</th>
        @else
            @if($core_attributes[1])<th>No Ticket</th>@endif
            ...
            @if($core_attributes[7])<th>Nama Engineer / Group</th>@endif
            @if($core_attributes[8])<th>Status Ticket</th>@endif
        @endif
    ...
    @foreach($tickets as $ticket)
        <tr>
            ...
            @if($core_attributes[0])
                <td>{{ $ticket->name }}</td>
                <td>{{ $ticket->task->creator->name ?? "-"}}</td>
                <td>{{ $ticket->task->creator->company->name ?? "-" }}</td>
                <td>{{ $ticket->created_at }}</td>
                <td>{{ $ticket->closed_at }}</td>
                <td>{{ $ticket->resolved_times }}</td>
                <td>{{ $ticket->assignment_operator }}</td>
                <td>{{ $ticket->status }}</td>
            @else
                @if($core_attributes[1])<td>{{ $ticket->name }}</td>@endif
                @if($core_attributes[2])<td>{{ $ticket->task->creator->name ?? "-"}}</td>@endif
                @if($core_attributes[3])<td>{{ $ticket->task->creator->company->name ?? "-" }}</td>@endif
                @if($core_attributes[4])<td>{{ $ticket->created_at }}</td>@endif
                @if($core_attributes[5])<td>{{ $ticket->closed_at }}</td>@endif
                @if($core_attributes[6])<td>{{ $ticket->resolved_times }}</td>@endif
                @if($core_attributes[7])<td>{{ $ticket->assignment_operator }}</td>@endif
                @if($core_attributes[8])<td>{{ $ticket->status }}</td>@endif
            @endif
