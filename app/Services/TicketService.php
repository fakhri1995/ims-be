...
class TicketService
{
    ...
    private function getTickets(Request $request, $admin)
    {
            ...
            foreach($tickets as $ticket){
                ...
                if($ticket->deadline){
                    ...
                    $date_deadline = date_create(explode(" ",$ticket->deadline)[0]);
    ...
    public function getTicket(Request $request, $admin)
    {
        try{
            ...
            // }
            if($admin){
                $ticket->original_deadline = $ticket->deadline ? $ticket->deadline : "-";
                $ticket->deadline = $ticket->deadline ? date("d M Y, H:i", strtotime($ticket->deadline)) : "-";
                $statuses = ['-','Overdue', 'Open', 'On progress', 'On hold', 'Completed', 'Closed', 'Canceled'];
            } else {
                $ticket->original_deadline = $ticket->deadline;
            ...
            foreach($ticket->tasks as $task){
                ...
                if($task->deadline){
                    ...
                    $deadline_detail = date_diff($date_now,$date_deadline);
                    if(date_format($date_now,"Ymd") <= date_format($date_deadline,"Ymd")){
                        if($deadline_detail->days == 0){
