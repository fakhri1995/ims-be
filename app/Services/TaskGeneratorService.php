...
class TaskGeneratorService
{
    ...
    private function newDeadline($last_deadline, $type)
    {
        if($type === 2){
    ...
    private function generateTask($previous_task, $new_deadline, $current_timestamp)
    {
        ...
        if($new_deadline > $previous_task->end_repeat_at) $repeat = 0;
        ...
        $task->first_deadline = $new_deadline;
    ...
    public function generateTasks($type)
    {
        ...
        foreach($periodic_needed_generated_tasks as $periodic_needed_generated_task){
            $deadline = $this->newDeadline($periodic_needed_generated_task->first_deadline, $type);
            $this->generateTask($periodic_needed_generated_task, $deadline, $current_timestamp);
        }
    }
}